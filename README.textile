...
虽说 Durov 说之后会开放 API，但显然，我已经等不及了。
...
p. 经过与友人多次测试，还发现文件名必须是 @WhatsApp Chat with name or phone number.txt@ 的格式，但文件名的用户名不用作分辨导入消息中哪方对应哪方。当聊天记录为一方手机号、另一方非手机号时手机号的那方将会成为对方，而手机号是自己的时也不会和 Telegram 账号手机号进行匹配（就是反了）。有三个用户名的时候，所有人的消息都将展示在对方一侧（推荐你导入群组）。
